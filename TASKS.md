...
- [ ] **Regenerate `contracts/sql/schema.sql`** — stale; missing `notification` (2026-06-02), TripMedia + role-default migrations, AND the new `chat_message` table (2026-07-13, see Done). CI `check-drift` fails. Run `task contract:sync-db` (needs the Docker Postgres up), commit the diff. **`chat_message` is a Go-read table** → after sync, Alaa reruns `task contract:generate` to add the chat sqlc repo (`contracts/sql/queries/chat/` is currently empty, so no repo exists yet — the chat-service history endpoints depend on it).
