...
CREATE TABLE public.parcels_delivery (
    ...
    CONSTRAINT parcels_delivery_schema_ver CHECK ((schema_version = ANY (ARRAY[1, 2]))),
    ...
    CONSTRAINT parcels_delivery_v1_required CHECK (((schema_version = 1) OR ((pickup_location_id IS NOT NULL) AND (delivery_location_id IS NOT NULL) AND (ready_window_start IS NOT NULL) AND (ready_window_end IS NOT NULL) AND (actual_weight_kg IS NOT NULL) AND (declared_value_eur_cents IS NOT NULL) AND (traveler_reward_eur_cents IS NOT NULL)))),
    CONSTRAINT parcels_delivery_v1_safety CHECK (((schema_version = 1) OR (description_is_accurate AND item_is_legal AND no_prohibited_goods AND declared_value_is_accurate AND customs_responsibilities_understood))),
...
CREATE TABLE public.trips_journey (
    ...
    traveler_id bigint NOT NULL,
    CONSTRAINT journey_distinct_endpoints CHECK ((NOT (start_location_id = destination_location_id)))
);
...
CREATE TABLE public.trips_journey_leg (
    ...
    CONSTRAINT journey_leg_distinct_endpoints CHECK ((NOT (origin_id = destination_id))),
